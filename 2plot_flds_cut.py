...
mybase = "/home/u21/davidrball/david/tristan_KSI/ple_runs/"

bangle0_base = mybase + "sig2_bangle10/flds.tot."
bangle1_base = mybase + 'sig2_bangle25/flds.tot.'
#bangle2_base = mybase + 'sig2_bangle15/output/flds.tot.'
...
for t in range(t0, tf):
    ...
    im0=ax0.imshow(b1,origin='lower',extent=extlist1,vmin=bmin1,vmax=bmax1/2.)
    im1=ax1.imshow(b2,origin='lower',extent=extlist1,vmin=bmin2,vmax=bmax2/2.)
    #im2=ax2.imshow(b3,origin='lower',extent=extlist1,vmin=bmin3,vmax=bmax3/2.)
    ax0.set_title('$\phi=\pi/10$ ($\phi > \phi_c$)')
    ax1.set_title('$\phi=\pi/25$ ($\phi < \phi_c$)')
    #ax2.set_title('$\phi=\pi/15$')
    ax1.set_yticks([])
    #ax2.set_yticks([])    
    ...
    plt.savefig('compare/2plot_phicrit_B'+tstr+'.png',dpi=300,bbox_inches='tight')
    plt.close()
    '''
    ...
    plt.savefig('compare/dens'+tstr+'.png',dpi=300,bbox_inches='tight')
    '''
